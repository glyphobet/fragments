...
class TestRenameCommand(CommandBase, PostInitCommandMixIn):
    ...
    def test_complex_move_into_existing_directory(self):
        init()
        file1_rel, file1_path = self._create_file(contents="GEE BER One", dir_name="GEE")
        file2_rel, file2_path = self._create_file(contents="GEE TWO ISH", dir_name="GEE")
        file3_rel, file3_path = self._create_file(contents="THREE BER ISH")
        file4_rel, file4_path = self._create_file(contents="GEE TWO ISH", dir_name="GEE")
        follow(file1_rel, file2_rel, file3_rel)
        commit(file1_rel, file2_rel, file3_rel)
        config = FragmentsConfig()
        os.mkdir(os.path.join(config.root, 'ISH'))
        self.assertEquals(rename(file3_rel, 'GEE', 'ISH'), [])
        config = FragmentsConfig()
        self.assertIn(os.path.join('ISH', file1_rel), config['files'])
        self.assertIn(os.path.join('ISH', file2_rel), config['files'])
        self.assertIn(os.path.join('ISH', file3_rel), config['files'])
        self.assertNotIn(os.path.join('ISH', file4_rel), config['files'])
        self.assertTrue(os.access(os.path.join('ISH', file1_rel), os.R_OK))
        self.assertTrue(os.access(os.path.join('ISH', file2_rel), os.R_OK))
        self.assertTrue(os.access(os.path.join('ISH', file3_rel), os.R_OK))
        self.assertTrue(os.access(os.path.join('ISH', file4_rel), os.R_OK))
        self.assertFalse(os.access(file1_rel, os.R_OK))
        self.assertFalse(os.access(file2_rel, os.R_OK))
        self.assertFalse(os.access(file3_rel, os.R_OK))
    ...
    def test_rename_directory(self):
        ...
        commit(file1_rel, file2_rel, file3_rel)
        config = FragmentsConfig()
        self.assertEquals(rename('GEE', 'ISH'), [])
        config = FragmentsConfig()
        self.assertIn(os.path.join('ISH', file1_rel[len('GEE')+1:]), config['files'])
        self.assertIn(os.path.join('ISH', file2_rel[len('GEE')+1:]), config['files'])
        self.assertIn(file3_rel, config['files'])
        self.assertNotIn(os.path.join('ISH', file4_rel[len('GEE')+1:]), config['files'])
        self.assertTrue(os.access(os.path.join('ISH', file1_rel[len('GEE')+1:]), os.R_OK))
        self.assertTrue(os.access(os.path.join('ISH', file2_rel[len('GEE')+1:]), os.R_OK))
        self.assertFalse(os.access(os.path.join('ISH', file3_rel[len('GEE')+1:]), os.R_OK))
        self.assertTrue(os.access(os.path.join('ISH', file4_rel[len('GEE')+1:]), os.R_OK))
        self.assertFalse(os.access(file1_rel, os.R_OK))
        self.assertFalse(os.access(file2_rel, os.R_OK))
